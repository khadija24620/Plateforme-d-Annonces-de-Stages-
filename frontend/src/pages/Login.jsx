...
export default function Login() {
  ...
  // ✅ handleLogin with real API call
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        else if (data.user.role === 'company') navigate('/my-offers');
        else navigate('/');
      }, 1200);
    } catch (err) {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .login-btn:not(:disabled):hover { background: ${C.accentHover}; transform: translateY(-1px); box-shadow: 0 8px 30px ${C.accentGlow}; }
        .login-btn:not(:disabled):active { transform: translateY(0); box-shadow: 0 2px 10px ${C.accentGlow}; }
        .login-btn:disabled { cursor: not-allowed; opacity: 0.7; }
        .login-btn::after { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,255,255,0.12) 0%, transparent 60%); pointer-events: none; }
        .divider-link { color: ${C.textMuted}; font-size: 0.78rem; text-decoration: none; transition: color 0.2s; font-family: 'DM Sans', sans-serif; }
        ...
        <div style={{
          ...
          {success ? (
            <div style={{ textAlign: 'center', padding: '20px 0', animation: 'fadeIn 0.4s ease both' }}>
              ...
              <p style={{ color: C.text, fontSize: '1.1rem', fontWeight: 600, marginBottom: 8, fontFamily: "'Fraunces', serif" }}>Connexion réussie</p>
            ...
            <>
              <div style={{ marginBottom: 36, animation: 'slideUp 0.5s cubic-bezier(0.16,1,0.3,1) 0ms both' }}>
                ...
                <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: '2rem', fontWeight: 700, color: C.text, lineHeight: 1.15, letterSpacing: '-0.02em', marginBottom: 10 }}>
                  Bon retour
                  <span style={{ display: 'block', color: C.accent, fontStyle: 'italic', fontWeight: 300 }}>parmi nous.</span>
                </h1>
                <p style={{ color: C.textSec, fontSize: '0.82rem', lineHeight: 1.6 }}>Connectez-vous pour accéder à votre espace stages et candidatures.</p>
              </div>

              <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <FloatingInput id="email" label="Adresse e-mail" type="email" value={email} onChange={setEmail} delay={80}
                  icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><rect x="2" y="4" width="20" height="16" rx="3"/><path d="m2 7 10 7 10-7"/></svg>}
                />
                <FloatingInput id="password" label="Mot de passe" type="password" value={password} onChange={setPassword} delay={140}
                ...
                <div style={{ textAlign: 'right', marginTop: -4, animation: 'slideUp 0.5s cubic-bezier(0.16,1,0.3,1) 180ms both' }}>
              ...
              <div style={{ marginTop: 32, paddingTop: 24, borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, animation: 'slideUp 0.5s cubic-bezier(0.16,1,0.3,1) 280ms both' }}>
                <span style={{ color: C.textMuted, fontSize: '0.78rem' }}>Pas encore de compte ?</span>
                <a href="/register" style={{ color: C.accent, fontSize: '0.78rem', fontWeight: 600, textDecoration: 'none', transition: 'opacity 0.2s' }}
